! function($, util, debug, server, navigation, lang, undefined) {
	...
	$.widget("apex.interactiveReport", 
	{
		...
		_create: function() {
			...
			0 === lRegion$.length && (lRegion$ = lIr$.wrap('<div id="' + lRegionId + '"></div>')), 
			this.idPrefix = this.options.regionId, 
			this.reportId = this._getElement("report_id").val(), t
			his.worksheetId = this._getElement("worksheet_id").val(), 
			this.viewMode = this._getElement("view_mode").val(), 
			this.region$ = lRegion$, 
			this.dialogDrop$ = this._getElement("dialog_js"), t
			his._getElement("worksheet_region").addClass(C_IRR), 
			this._initOnCreate(), this._on(this.region$, {
		...
		_action: function(pAction, pData) {
			...
			this.currentAction = pAction, this._get(lData)
		},
		...
		_aggregateControl: function() {
			var lAggregateBy = this._getDialogElement("aggregate_by").val(),
				lIsAllColumns = -1 !== $.inArray(lAggregateBy, ["COUNT", "COUNT_DISTINCT"]);
			this._getDialogElement("all_columns_container").toggle(lIsAllColumns), this._getDialogElement("number_columns_container").toggle(!lIsAllColumns)
		},
		_aggregateSave: function() {
			var lColumn, lAggregateBy = this._getDialogElement("aggregate_by").val(),
				lAggregation = this._getDialogElement("aggregation").val();
			lColumn = -1 !== $.inArray(lAggregateBy, ["COUNT", "COUNT_DISTINCT"]) ? this._getDialogElement("all_columns").val() : this._getDialogElement("number_columns").val(), this._action("SAVE_AGGREGATE", {
				id: lAggregation,
				value: lAggregateBy,
				x05: lColumn
			})
		...
		_chartClear: function() {
			this.options.reportViewMode = "REPORT", this._action("DELETE_CHART")
		},
		_chartControl: function() {
			$("td." + C_IRR_CHART_ATTR_NOT_FOR_PIE, this._getElement("dialog_js")).toggle(!this._getDialogElement("chart_type_2").prop("checked"))
		},
		_chartSave: function() {
			this.suppressUpdate = !0, this._action("SAVE_CHART", {
		...
		_columnOrder: function(pDirection) {
			var lFArrays;
			"ASC" === pDirection || "DESC" === pDirection ? this._action("COLUMN_ORDER", {
				f01: this.lastColumnId,
				f02: pDirection
			}) : (lFArrays = this._utilGetFormElAttributes(), this._action("SORT", {
				f01: lFArrays.ids,
				f02: lFArrays.values
			}))
		},
		...
		_columnHide: function(pColumnId) {
			var lValue;
			lValue = pColumnId ? pColumnId : this.currentColumnId, this._action("HIDE", {
				id: lValue
			})
		},
		_columnSearchShow: function(pData) {
			var i, lIsAllColumns, lItems = [],
				lChoices = [],
				that = this,
				lRows = $.parseJSON(pData).row;
			for (i = 0; i < lRows.length; i++) lIsAllColumns = "0" === lRows[i].C, lIsAllColumns ? (lItems.push({
				type: "action",
				label: lRows[i].D,
				value: lRows[i].R,
				action: function() {
					that._getElement("column_search_root").removeClass("is-active"), that._getElement("column_search_current_column").val(""), that._getElement("search_field").attr({
						title: "Search",
						placeholder: ""
					})
				}
			...
			lItems.push({
				...
				set: function(pValue) {
					var lLabel, lValue, lChoices = this.choices;
					for (i = 0; i < lChoices.length; i++)
						if (lChoices[i].value === pValue) {
							lLabel = format("SEARCH_COLUMN", lChoices[i].label), lValue = lChoices[i].value, that._getElement("column_search_root").addClass("is-active"), that._getElement("column_search_current_column").val(lValue), that._getElement("search_field").attr({
								title: lLabel,
								placeholder: lLabel
							});
							break
				...
				choices: lChoices
			}), this.searchMenu.items = lItems, this.searchMenuCallback(), this.searchMenuCallback = null, this.searchMenu = null
		},
		...
		_computationSave: function() {
			...
			lValidate || (this.suppressUpdate = !0, this._action("SAVE_COMPUTATION", {
				id: this._getDialogElement("comp_id").val(),
		...
		_computationShow: function(pId) {
			var lId;
			lId = pId ? pId : this.computationId, this._controlsGet("SHOW_COMPUTATION", lId), this.computationId = null
		},
		_controlBreakOn: function(pColumnId) {
			pColumnId && (this.currentColumnId = pColumnId), this._action("BREAK", {
				id: this.currentColumnId
			})
		},
		_controlBreakSave: function() {
			var i, lIds = [];
			for (i = 1; 7 > i; i++) lIds.push(this._getId("column") + "_0" + i), lIds.push(this._getId("enable") + "_0" + i);
		...
		_controlsFormatMask: function(pID) {
			this.suppressUpdate = !0, this.tempReturnElement$ = $(getIdSelector(pID)), this._controlsGet("FORMAT_MASK_LOV")
		},
		_controlsGet: function(pControl, pID) {
			this._dialogReset(), this.currentAction = "CONTROL", this.currentControl = pControl, pID && (this.currentColumnId = pID), this._get({
				widgetMod: "CONTROL",
		...
		_controlsNarrow: function(pID) {
			this.suppressUpdate = !0, this.tempReturnElement$ = $(getIdSelector(pID)), this._controlsGet("NARROW", this._getDialogElement("column_name").val())
		},
		...
		_dialogCheck: function(pContainer, pCallback) {
			...
			pContainer && (lContainer = pContainer), pCallback && (lCallback = pCallback), this._on(this.document, {
				click: function(pEvent) {
					$(pEvent.target).closest(lContainer).length || lCallback.call(this)
				}
			})
		},
			...
			$(pSelector).each(function() {
				var lColorPicker$ = $(this).ColorPicker({
					...
					onSubmit: function(pHsb, pHex, pRgb, pElement) {
						...
						lElement$.val("#" + pHex.toUpperCase()).trigger("change").ColorPickerHide()
					},
					onBeforeShow: function() {
						$(this).ColorPickerSetColor(this.value)
					},
					onShow: function(pElement) {
						return $(pElement).fadeIn("fast"), !1
					},
					onHide: function(pElement) {
						return $(pElement).fadeOut("fast"), !1
					}
				}).ColorPickerHide();
				that._on(lColorPicker$, {
					...
					change: function(pEvent) {
						...
						lTarget.value = lTarget.value.toUpperCase(), $(getIdSelector(lTarget.id + "_preview")).css("background", lTarget.value)
					}
				}), that._on(getIdSelector(this.id + "_picker"), {
					click: function(pEvent) {
						lColorPicker$.ColorPickerShow(), pEvent.preventDefault()
					}
		...
		_dialogColumnCheck: function() {
			...
			$(lCurrentOperator$).siblings().hide().end().show(), this._dialogOperatorCheck(lCurrentOperator$)
		},
		_dialogOpen: function(pInit) {
			function displayButton(pId, pAction, pLabel, pHot, pClose) {
				var lLabel, lStyle;
				lLabel = pLabel ? pLabel : MSG.BUTTON.APPLY, pHot && (lStyle = C_UI_BUTTON_HOT), lButtons.push({
					text: lLabel,
					"class": lStyle,
					click: function() {
						pId ? pAction.call(that, pId) : pAction.call(that), pClose && that._getElement("dialog_js").dialog("close")
					}
				})
			}
			var lIRRDlg$, lTitle, lId, lError, lButtons, that = this;
			switch (lButtons = [{
				...
				case "SHOW_COLUMN":
					lTitle = MSG.DIALOG_TITLE.SELECT_COLUMNS, displayButton(null, that._columnDisplay, null, !0, !0);
					break;
				case "SHOW_FILTER":
					lTitle = MSG.DIALOG_TITLE.FILTER, lId = that._getDialogElement("filter_id").val(), lId && displayButton(lId, that._filterDelete, MSG.BUTTON.DELETE, !1, !0), displayButton(lId, that._filterSave, null, !0, !1);
					break;
				case "SHOW_HIGHLIGHT":
					lTitle = MSG.DIALOG_TITLE.HIGHLIGHT, lId = that._getDialogElement("highlight_id").val(), lId && displayButton(lId, that._highlightClear, MSG.BUTTON.DELETE, !1, !0), displayButton(lId, that._highlightSave, null, !0, !1);
					break;
				case "SHOW_ORDERING":
					lTitle = MSG.DIALOG_TITLE.SORT, displayButton(this, that._columnOrder, null, !0, !0);
					break;
				case "SHOW_CTRL_BREAK":
					lTitle = MSG.DIALOG_TITLE.CONTROL_BREAK, displayButton(null, that._controlBreakSave, null, !0, !0);
					break;
				case "SHOW_COMPUTATION":
					lTitle = MSG.DIALOG_TITLE.COMPUTATION, lId = that._getDialogElement("comp_id").val(), lId && displayButton(null, that._computationClear, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._computationSave, null, !0, !1);
					break;
				case "SHOW_AGGREGATE":
					lTitle = MSG.DIALOG_TITLE.AGGREGATE, lId = that._getDialogElement("aggregation").val(), lId && displayButton(lId, that._aggregateClear, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._aggregateSave, null, !0, !0);
					break;
				case "SHOW_CHART":
					lTitle = MSG.DIALOG_TITLE.CHART, lId = that._getDialogElement("chart_type_hidden").val(), lId && displayButton(null, that._chartClear, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._chartSave, null, !0, !1);
					break;
				case "SHOW_GROUP_BY":
					lTitle = MSG.DIALOG_TITLE.GROUP_BY, lId = that._getDialogElement("group_by_id").val(), lId && displayButton(null, that._groupByRemove, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._groupBySave, null, !0, !1);
					break;
				case "SHOW_GROUP_BY_SORT":
					lTitle = MSG.DIALOG_TITLE.GROUP_BY_SORT, displayButton(this, that._groupBySort, null, !0, !0);
					break;
				case "SHOW_PIVOT":
					lTitle = MSG.DIALOG_TITLE.PIVOT, lId = that._getDialogElement("pivot_id").val(), lId && displayButton(null, that._pivotRemove, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._pivotSave, null, !0, !1);
					break;
				case "SHOW_PIVOT_SORT":
					lTitle = MSG.DIALOG_TITLE.PIVOT_SORT, displayButton(this, that._pivotSort, null, !0, !0);
					break;
				case "SHOW_FLASHBACK":
					lTitle = MSG.DIALOG_TITLE.FLASHBACK, lId = that._getDialogElement("flashback_time").val(), lId && displayButton(null, that._flashbackClear, MSG.BUTTON.DELETE, !1, !0), displayButton(null, that._flashbackSave, null, !0, !1);
					break;
				case "SAVE_REPORT":
					lTitle = MSG.DIALOG_TITLE.SAVE_REPORT, displayButton(null, that._save, null, !0, !1);
					break;
				case "SHOW_RENAME":
					lTitle = MSG.DIALOG_TITLE.RENAME_REPORT, displayButton(that.reportId, that._save, null, !0, !1);
					break;
				case "SHOW_DELETE":
					lTitle = MSG.DIALOG_TITLE.DELETE_REPORT, displayButton(null, that._remove, null, !0, !0);
					break;
				case "SHOW_SAVE_DEFAULT":
					lTitle = MSG.DIALOG_TITLE.SAVE_DEFAULT_REPORT, displayButton(null, that._saveDefault, null, !0, !1);
					break;
				case "SHOW_RENAME_DEFAULT":
					lTitle = MSG.DIALOG_TITLE.RENAME_DEFAULT_REPORT, displayButton("RENAME_DEFAULT", that._saveDefault, null, !0, !1);
					break;
				case "SHOW_DELETE_DEFAULT":
					lTitle = MSG.DIALOG_TITLE.DELETE_DEFAULT_REPORT, displayButton("DELETE_DEFAULT", that._remove, null, !0, !0);
					break;
				case "SHOW_RESET":
					lTitle = MSG.DIALOG_TITLE.RESET, displayButton(null, that._reset, null, !0, !0);
					break;
				case "SHOW_DOWNLOAD":
					lTitle = MSG.DIALOG_TITLE.DOWNLOAD;
					break;
				case "SHOW_NOTIFY":
					lTitle = MSG.DIALOG_TITLE.SUBSCRIPTION, lId = that._getDialogElement("notify_id").val(), lError = that._getDialogElement("email_not_configured").text(), lId && displayButton(null, that._notifyClear, MSG.BUTTON.DELETE, !1, !0), lError || displayButton(null, that._notifySave, null, !0, !1)
			}
			lIRRDlg$ = this._getElement("dialog_js").dialog({
				...
				close: function() {
					that._off(lIRRDlg$), lIRRDlg$.dialog("destroy")
				}
			}), pInit && pInit.call()
		},
		...
		_emailSend: function() {
			var lIds, lRequired = [this._getId("email_to")],
				lValidate = $f_get_emptys(lRequired, "error", "");
			lValidate || (lIds = [this._getId("email_to"), this._getId("email_cc"), this._getId("email_bcc"), this._getId("email_subject"), this._getId("email_body")], this._action("SEND_EMAIL", {
				f01: lIds,
				f02: this._utilGetFormElValues(lIds)
			}), this._getElement("dialog_js").dialog("close"))
		},
		_emailShow: function() {
			var lButtons, that = this,
			...
			lEmailContainer$.show(), lEmailButton$.show(), lEmailButton$.is(":visible") && (lButtons = [{
				text: MSG.BUTTON.CANCEL,
				click: function() {
		...
		_filterSave: function(pThis) {
			var l_OB, lTemp, lActionMod = "ADD",
				lFilterTypeId = this._getId("filter_type");
			if (pThis ? (this.currentColumnId = pThis, lActionMod = "UPDATE") : pThis = this._getDialogElement("column_name").val(), "COLUMN" === $v(lFilterTypeId)) {
				if (l_OB = this._dialogValidate(), !l_OB) return;
				lTemp = [lFilterTypeId, l_OB.col.id, l_OB.col_opt.id, l_OB.form_items[0], l_OB.form_items[1], this._getId("expression3")]
			} else lTemp = [lFilterTypeId, this._getId("filter_expr"), this._getId("filter_name")];
			this.suppressUpdate = !0, this._action("FILTER", {
		...
		_flashbackSave: function() {
			this.suppressUpdate = !0, this._action("FLASHBACK_SET", {
		...
		_functionControl: function(pThis) {
			var lElId, lRowNum, lIsNumberColumn, lNumFunctions = ["SUM", "AVG", "MAX", "MIN", "MEDIAN", "RATIO_TO_REPORT_SUM"];
			pThis && (lElId = pThis.id, lRowNum = lElId.substr(lElId.length - 2), lIsNumberColumn = -1 !== $.inArray($(pThis).val(), lNumFunctions), this._getDialogElement("number_columns_container_" + lRowNum).toggle(lIsNumberColumn), this._getDialogElement("all_columns_container_" + lRowNum).toggle(!lIsNumberColumn))
		},
		_get: function(pData) {
			function _return(pData) {
				var lSetFocusTo$, lValidatingActions, dialogInit, lData$, lToolbar$, lContent$;
				if ("CONTROL" === that.currentAction) {
					if ("SORT_WIDGET" === that.currentControl) that._sortWidgetShow(pData);
					else if ("INFO" === that.currentControl) that._helpWidgetShow(pData);
					else if ("NARROW" === that.currentControl) lSetFocusTo$ = that._inlinePicker("NARROW", pData);
					else if ("FORMAT_MASK_LOV" === that.currentControl) lSetFocusTo$ = that._inlinePicker("FORMAT_MASK_LOV", pData);
					else if ("SEARCH_COLUMN" === that.currentControl) that._columnSearchShow(pData);
					else if ("SHOW_DETAIL" === that.currentControl) that._singleRowViewShow(pData);
					else {
						switch (that.currentControl) {
							case "SHOW_FILTER":
								dialogInit = function() {
									that._dialogColumnCheck(), that._dialogColumnHandlers(), that._on(that._getDialogElement("filter_type"), {
										"click input": function(pEvent) {
											var lIsColumnFilter = "COLUMN" === pEvent.target.value;
											that._getDialogElement("column_filter").toggle(lIsColumnFilter), that._getDialogElement("row_filter").toggle(!lIsColumnFilter)
										}
									}), that._on(that._getDialogElement("filter_expr_clear"), {
										click: function(pEvent) {
											that._getDialogElement("filter_expr").val(""), pEvent.preventDefault()
										}
										...
										"click tr.a-IRR-row-filter-column": function(pEvent) {
											...
											that._dialogComp(that._getId("filter_expr"), lColumnId), pEvent.preventDefault()
										}
										...
										"click tr.a-IRR-row-filter-operator": function(pEvent) {
											...
											that._dialogComp(that._getId("filter_expr"), lOperator), pEvent.preventDefault()
										}
									})
								};
								break;
							case "SHOW_HIGHLIGHT":
								dialogInit = function() {
									that._dialogColumnCheck(), that._dialogColumnHandlers(), that._dialogColorPicker(that._getIdSelector("bg_color") + "," + that._getIdSelector("font_color"))
								};
								break;
							case "SHOW_CHART":
								dialogInit = function() {
									that._chartControl(), that._on(that._getDialogElement("chart_type"), {
							...
							case "SHOW_COMPUTATION":
								dialogInit = function() {
									that._on(that._getDialogElement("comp_id"), {
										...
										click: function(pEvent) {
											that._controlsFormatMask(that._getId("format_mask")), pEvent.preventDefault()
										}
									}), that._on(that._getDialogElement("computation_expr_clear"), {
										click: function(pEvent) {
											that._getDialogElement("computation_expr").val(""), pEvent.preventDefault()
										}
									}), that._on(that._getDialogElement("computation_columns"), {
										"click tr.a-IRR-computation-column": function(pEvent) {
											var lColumnId = $(pEvent.currentTarget).data("column-id");
											that._dialogComp(that._getId("computation_expr"), lColumnId), pEvent.preventDefault()
										}
										...
										"click td.a-IRR-key": function(pEvent) {
											...
											that._dialogComp(that._getId("computation_expr"), lKey), pEvent.preventDefault()
										}
										...
										"click tr.a-IRR-computation-function": function(pEvent) {
											...
											that._dialogComp(that._getId("computation_expr"), lFunction), pEvent.preventDefault()
										}
									})
								};
								break;
							case "SHOW_COLUMN":
								dialogInit = function() {
									if (that.g_Shuttlep_v01 = null, !flowSelectArray) var flowSelectArray = [];
									flowSelectArray[2] = $x(that._getId("shuttle_left")), flowSelectArray[1] = $x(that._getId("shuttle_right")), window.g_Shuttlep_v01 = new dhtml_ShuttleObject(flowSelectArray[2], flowSelectArray[1])
								};
								break;
							case "SAVE_REPORT":
								dialogInit = function() {
									...
									}), that.options.saveReportCategory && that._on(that._getDialogElement("report_category"), {
										change: function(pEvent) {
							...
							case "SHOW_RENAME_DEFAULT":
								dialogInit = function() {
									...
									}), that._saveDefaultTypeCheck($v(that._getId("default_type")))
								};
								break;
							case "SHOW_AGGREGATE":
								dialogInit = function() {
							...
							case "SHOW_DOWNLOAD":
								dialogInit = function() {
									that._on(that._getDialogElement("download_EMAIL"), {
										click: function(pEvent) {
											that._emailShow(), pEvent.preventDefault()
										}
									})
								};
								break;
							case "SHOW_GROUP_BY":
								dialogInit = function() {
									that._on(that._getDialogElement("add_column"), {
										click: function(pEvent) {
											this._utilAddMoreColumn(this._groupByMaxValue, C_IRR_GROUP_BY_COLUMN), pEvent.preventDefault()
										}
										...
										"click a.a-IRR-format-mask-picker": function(pEvent) {
											that._controlsFormatMask($(pEvent.currentTarget).data("picker-for")), pEvent.preventDefault()
										}
									}), that._on(that._getDialogElement("add_function"), {
										click: function(pEvent) {
											this._utilAddMoreFunction(this._groupByMaxValue), pEvent.preventDefault()
										}
									})
								};
								break;
							case "SHOW_PIVOT":
								dialogInit = function() {
									that._on(that._getDialogElement("add_pivot_column"), {
										click: function(pEvent) {
											this._utilAddMoreColumn(this._pivotMaxValue, C_IRR_PIVOT_COLUMN), pEvent.preventDefault()
										}
									}), that._on(that._getDialogElement("add_row_column"), {
										click: function(pEvent) {
											this._utilAddMoreColumn(this._pivotMaxValue, C_IRR_ROW_COLUMN), pEvent.preventDefault()
										}
										...
										click: function(pEvent) {
											this._utilAddMoreFunction(this._pivotMaxValue), pEvent.preventDefault()
										}
									})
								}
						}
						that._getElement("dialog_js").html(pData), that._dialogOpen(dialogInit)
					}
					lSetFocusTo$ === undefined && (lSetFocusTo$ = $(":input:visible", that._getElement("dialog_js"))), lSetFocusTo$.length > 0 && lSetFocusTo$[0].focus()
				} else that.suppressUpdate ? (lValidatingActions = ["SAVE_NOTIFY", "SAVE_COMPUTATION", "FLASHBACK_SET", "SAVE_HIGHLIGHT", "FILTER", "GROUP_BY_SAVE", "SAVE_PIVOT", "SAVE_CHART"], $.inArray(that.currentAction, lValidatingActions) > -1 && (that.lastFunction = function() {
					that._validAction(pData)
				}), that.ajaxBusy = !1, that.suppressUpdate = !1, that.currentAction = null) : (lData$ = $($u_js_temp_drop()), lData$.html(pData), lToolbar$ = lData$.find(that._getIdSelector("toolbar_controls")), lContent$ = lData$.find(that._getIdSelector("content")), that._getElement("toolbar_controls", that.element).replaceWith(lToolbar$), that._getElement("content", that.element).replaceWith(lContent$), that.options.currentRowsPerPage = 1 * that._getElement("row_select").val(), that.viewMode = that._getElement("view_mode").val(), that._initToolbar(), that._initControlPanel(), that.currentAction = null, that.currentControl = null, that.reportId = that._getElement("report_id").val(), that._singleRowViewHide(), that._getElement("full_view").show(), lData$.remove(), that._initFixedHeader(!0), that.element.find(":focusable")[0].focus())
			}

			function _finishedLoading() {
				that.lastFunction && (that.lastFunction(), that.lastFunction = !1), that.ajaxBusy = !1, "SEARCH_COLUMN" === that.currentControl && that.searchMenuCallback && (that.searchMenuCallback(!1), that.searchMenuCallback = null, that.searchMenu = null)
			}
			var lData, lOptions, lCallIssued, that = this,
				lTriggerRefresh = !1;
			"INFO" !== pData.widgetAction && this._sortWidgetHide(), pData.widgetMod && !this.suppressUpdate && (-1 !== $.inArray(pData.widgetMod, ["ACTION", "PULL"]) ? lTriggerRefresh = !0 : "CONTROL" === pData.widgetMod && pData.widgetAction && "SHOW_DETAIL" === pData.widgetAction && (lTriggerRefresh = !0)), lData = $.extend({
			...
			}, pData), delete lData.widgetMod, delete lData.widgetAction, delete lData.widgetActionMod, delete lData.id, delete lData.value, "RESET" === lData.p_widget_action && delete lData.p_widget_num_return, lOptions = {
				dataType: "html",
			...
			}, lCallIssued = server.plugin(this.options.ajaxIdentifier, lData, lOptions), lCallIssued || (this.ajaxBusy = !1)
		},
		_getId: function(pId) {
		...
		_getElement: function(pId, pContext$) {
			var lContext$;
			return lContext$ = pContext$ ? pContext$ : "dialog_js" === pId || /^sort_widget.*$/.test(pId) ? apex.gPageContext$ : this.element, $(this._getIdSelector(pId), lContext$)
		},
		...
		_groupBySave: function() {
			...
			this.suppressUpdate = !0, this._action("GROUP_BY_SAVE", {
				f01: lFArrays.ids,
				f02: lFArrays.values
			})
		},
		_groupBySort: function(pThis) {
			var lFArrays, lGroupById = this._getDialogElement("group_by_id").val();
			"ASC" === pThis || "DESC" === pThis ? (this.lastColumnId = pThis.id, this._action("GROUP_BY_COLUMN_SORT", {
				id: lGroupById,
				f01: this.lastColumnId,
				f02: pThis
			})) : (lFArrays = this._utilGetFormElAttributes(), this._action("GROUP_BY_SORT", {
				id: lGroupById,
				f01: lFArrays.ids,
				f02: lFArrays.values
			}))
		...
		_helpWidgetShow: function(pData) {
			this._getElement("sort_widget_help").html(pData).show(), this._getElement("sort_widget_action_help").find("button.a-IRR-sortWidget-button").addClass("is-active"), this.lastColumnId = this.currentColumnId, this._dialogCheck(null, this._sortWidgetHide)
		},
		_highlightClear: function(pHighlightId) {
			var lHighlightId;
			lHighlightId = pHighlightId ? pHighlightId : $x("HIGHLIGHT_ID").value, this._action("CLEAR_HIGHLIGHT", {
				id: lHighlightId
			})
		},
		_highlightSave: function() {
			var lFArrays, l_OB = this._dialogValidate();
			l_OB && (this.suppressUpdate = !0, lFArrays = this._utilGetFormElAttributes(), this._action("SAVE_HIGHLIGHT", {
				f01: lFArrays.ids,
				f02: lFArrays.values
			}), l_OB = null)
		},
		...
		_initControlPanel: function() {
			function _expand() {
				that.options.isControlPanelCollapsed && (that.options.isControlPanelCollapsed = !1, that.suppressUpdate = !0, that._action("CONTROL_MIN", {
					value: "N"
				})), that._getElement("control_panel_summary").hide()
			}

			function _collapse() {
				that.options.isControlPanelCollapsed || (that.options.isControlPanelCollapsed = !0, that.suppressUpdate = !0, that._action("CONTROL_MIN", {
					value: "Y"
		...
		_initFixedHeader: function(pWindowResize) {
			var lContainerClass, o = this.options;
			if (!$x("pScreenReaderMode") && ("PAGE" === o.fixedHeader || "REGION" === o.fixedHeader) && 0 === $(".a-IRR-header--group", this.element).length) {
				switch (this.viewMode) {
					case "REPORT":
						"REPORT" === o.reportViewMode && (lContainerClass = "a-IRR-reportView");
						break;
					case "GROUP_BY":
						lContainerClass = "a-IRR-groupByView"
				}
				if (lContainerClass) {
					if (0 === $(this.element).find(".a-IRR-iconViewTable").length) switch (o.fixedHeader) {
						case "PAGE":
							$("." + lContainerClass, this.element).setTableHeadersAsFixed(), $(".js-stickyTableHeader", this.element).stickyWidget({
								toggleWidth: !0,
								stickToBottom: !0
							});
							break;
					...
					pWindowResize && $(window).trigger("apexwindowresized")
				}
			}
		},
		_initMenus: function() {
			var that = this,
				o = this.options,
				rowsPerPageChoices = [1, 5, 10, 15, 20, 25, 50, 100, 1e3],
				actionMenu = {
					items: [{
						id: "irColumn",
						type: "action",
						label: msg("SELECT_COLUMNS"),
						hide: !0,
						...
						menu: {
							items: [{
								type: "radioGroup",
								set: function(v) {
									that.options.currentRowsPerPage = 1 * v, that._search("SEARCH")
								},
								get: function() {
			...
			o.toolbar && o.actionsMenu && that._getElement("actions_menu").menu(actionMenu)
		},
		_initOnCreate: function() {
			var o = this.options;
			o.toolbar && this._initToolbar(!0), this._initControlPanel(), $(document.body).append(this._getElement("sort_widget")), this._on(this.element, {
				"click a.a-IRR-headerLink": function(pEvent) {
					"REPORT" === this.viewMode ? this._controlsGet("SORT_WIDGET", $(pEvent.currentTarget).data("column")) : "GROUP_BY" === this.viewMode ? this._controlsGet("SHOW_GROUP_BY_SORT") : "PIVOT" === this.viewMode && this._controlsGet("SHOW_PIVOT_SORT"), pEvent.preventDefault()
				}
			}), this._on(this.element, {
				"click button.a-IRR-button--pagination": function(pEvent) {
					o.detailLink && "SHOW_DETAIL" === this.currentControl ? this._controlsRow($(pEvent.currentTarget).data("row-id")) : o.pagination && this._paginate($(pEvent.currentTarget).data("pagination"))
				},
				"click a.a-IRR-pagination-reset": function(pEvent) {
					this._search("SEARCH"), pEvent.preventDefault()
				}
			}), o.pagination && this._on(this.element, {
				"click a.a-IRR-pagination-reset": function(pEvent) {
					this._search("SEARCH"), pEvent.preventDefault()
				}
			}), o.detailLink && this._on(this.element, {
				"click a.a-IRR-detail-row": function(pEvent) {
					this._controlsRow($(pEvent.currentTarget).data("row-id")), pEvent.preventDefault()
				}
		...
		_initToolbar: function(pOnCreate) {
			var o = this.options,
				that = this;
			o.searchField && (this._on(this._getElement("search_field"), {
				keydown: function(pEvent) {
					pEvent.which === $.ui.keyCode.ENTER && (this._search("SEARCH"), pEvent.preventDefault())
				}
			}), this._on(this._getElement("search_button"), {
				click: function(pEvent) {
					this._search("SEARCH"), pEvent.preventDefault()
				}
			}), o.columnSearch && pOnCreate && this._getElement("column_search_drop").menu({
				asyncFetchMenu: function(menu, callback) {
					that.searchMenu = menu, that.searchMenuCallback = callback, that._controlsGet("SEARCH_COLUMN")
				},
				afterClose: function(event, data) {
					data.actionTaken && setTimeout(function() {
						that._getElement("search_field").focus()
					}, 100)
				},
				items: []
			})), o.rowsPerPageSelect && this._on(this._getElement("row_select"), {
				change: function(pEvent) {
					var lRowValue = pEvent.target.value;
					this._search("SEARCH", lRowValue), pEvent.preventDefault()
				}
			}), o.reportsSelect && this._on(this._getElement("saved_reports"), {
				change: function(pEvent) {
					var lReportId = pEvent.target.value;
					this._pull("REPORT_CHANGED", lReportId), pEvent.preventDefault()
				}
			...
			}), o.actionsMenu && pOnCreate && this._initMenus()
		},
		_inlinePicker: function(pControl, pData) {
			var i, lColValuesDrop$, out = util.htmlBuilder(),
				lData = $.parseJSON(pData),
				lIcon$ = $("[data-picker-for=" + this.tempReturnElement$.attr("id") + "]", this._getElement("dialog_js"));
			for (out.markup("<div").attr("id", this._getId("col_values_drop")).attr("class", "a-IRR-col-values-drop").markup(">"), i = 0; i < lData.row.length; i++) out.markup("<a").attr("href", "#").attr("class", C_IRR_COL_VALUE).attr("data-return-value", lData.row[i].R).markup(">").markup(lData.row[i].D).markup("</a>");
			return out.markup("</div>"), lIcon$.after(out.toString()), lColValuesDrop$ = this._getDialogElement("col_values_drop"), "NARROW" === pControl ? this._on(lColValuesDrop$, {
				"click a.a-IRR-col-value": function(pEvent) {
					var lCurrentValue, lSelectedValue = $(pEvent.currentTarget).data("returnValue"),
						lCurrentOperator = this._dialogCurrentOperator().val();
					"in" === lCurrentOperator || "not in" === lCurrentOperator ? (lCurrentValue = this.tempReturnElement$.val(), lCurrentValue && (lSelectedValue = lCurrentValue + "," + lSelectedValue), this.tempReturnElement$.val(lSelectedValue)) : (this.tempReturnElement$.val(lSelectedValue), lColValuesDrop$.remove(), this.tempReturnElement$.focus()), pEvent.preventDefault()
				}
			}) : this._on(lColValuesDrop$, {
				"click a.a-IRR-col-value": function(pEvent) {
					var lSelectedValue = $(pEvent.currentTarget).data("returnValue");
					this.tempReturnElement$.val(lSelectedValue), lColValuesDrop$.remove(), this.tempReturnElement$.focus(), pEvent.preventDefault()
				}
			}), this.suppressUpdate = !1, this.currentAction = null, this._on(lColValuesDrop$, {
				"keydown a": function(pEvent) {
					var lAnchorList$ = lColValuesDrop$.find("a"),
						lCurrentIndex = lAnchorList$.index($(pEvent.target)),
						lAnchorListLength = lAnchorList$.length - 1;
					switch (lAnchorList$.attr("tabindex", -1), pEvent.which) {
						case $.ui.keyCode.DOWN:
							lAnchorListLength > lCurrentIndex ? lAnchorList$.eq(lCurrentIndex + 1).focus() : lAnchorList$.eq(0).focus(), pEvent.preventDefault();
							break;
						case $.ui.keyCode.UP:
							lCurrentIndex > 0 ? lAnchorList$.eq(lCurrentIndex - 1).focus() : lAnchorList$.eq(lAnchorListLength).focus(), pEvent.preventDefault();
							break;
						case $.ui.keyCode.TAB:
							lColValuesDrop$.remove(), lIcon$.focus(), pEvent.preventDefault();
							break;
						case $.ui.keyCode.ESCAPE:
							lColValuesDrop$.remove(), lIcon$.focus(), pEvent.stopImmediatePropagation()
					}
				}
			}), this._dialogCheck(this._getIdSelector("col_values_drop"), function() {
				lColValuesDrop$.remove()
			}), lColValuesDrop$.find("a:first")
		},
		...
		_notifySave: function() {
			var lIds = [this._getId("notify_id"), this._getId("email_address"), this._getId("email_subject"), this._getId("notify_interval"), this._getId("start_date"), this._getId("end_date")];
			this.suppressUpdate = !0, this._action("SAVE_NOTIFY", {
				f01: lIds,
				f02: this._utilGetFormElValues(lIds)
			})
		},
		...
		_pivotSave: function() {
			var lFArrays = this._utilGetFormElAttributes();
			this.suppressUpdate = !0, this._action("SAVE_PIVOT", {
				f01: lFArrays.ids,
				f02: lFArrays.values
			})
		},
		_pivotSort: function(pThis) {
			var lFArrays, lPivotId = this._getDialogElement("pivot_id").val();
			"ASC" === pThis || "DESC" === pThis ? (this.lastColumnId = pThis.id, this._action("PIVOT_COLUMN_SORT", {
				id: lPivotId,
				f01: this.lastColumnId,
				f02: pThis
			})) : (lFArrays = this._utilGetFormElAttributes(), this._action("PIVOT_SORT", {
				id: lPivotId,
				f01: lFArrays.ids,
				f02: lFArrays.values
			}))
		...
		_pull: function(pAction, pReportId, pData) {
			...
			pReportId && (this.reportId = pReportId), this.currentAction = pAction, this._get(lData)
		},
		_remove: function(pAction) {
			var lAction = "DELETE";
			pAction && (lAction = pAction), this._action(lAction)
		},
		_reportView: function(pMode) {
			this.options.reportViewMode = pMode, this._action("VIEW_REPORT", {
		...
		_saveCategoryCheck: function(pThis) {
			...
			"new" === pThis.value ? $("<input/>", {
				id: lNewCategoryId,
				type: "text",
				title: msg("NEW_CATEGORY_LABEL"),
				name: ""
			}).insertAfter(pThis) : this._getDialogElement("new_category").remove()
		},
		_saveDefault: function(pAction) {
			var lValidate, lIds = [],
				...
				lReportNameId = this._getId("report_name");
			"ALTERNATIVE" === lDefaultType && (lValidate = $f_get_emptys([lReportNameId], "error", "")), lValidate || (pAction && (lAction = pAction), lIds.push(lDefaultTypeId), lValues.push(lDefaultType), "ALTERNATIVE" === lDefaultType && (lIds.push(lReportNameId), lValues.push(this._getDialogElement("report_name").val())), this._action(lAction, {
				f01: lIds,
				f02: lValues
			}), this._getElement("dialog_js").dialog("close"))
		},
		_saveDefaultTypeCheck: function(pDefaultType) {
			var lIsAlternativeDefault = "ALTERNATIVE" === pDefaultType;
			this._getDialogElement("report_name").closest("tr").toggle(lIsAlternativeDefault)
		},
		_search: function(pThis, pRows) {
			var lData, lFArrays, o = this.options,
				lSearch = this._getElement("search_field").val();
			pRows && (o.rowsPerPage || o.rowsPerPageSelect) && (o.currentRowsPerPage = 1 * pRows), lFArrays = this._utilGetFormElAttributes(this._getId("toolbar_controls")), lData = {
				f01: lFArrays.ids,
				f02: lFArrays.values
			}, "" === lSearch ? this._pull(null, this.reportId, lData) : this._action("QUICK_FILTER", lData)
		},
		_singleRowViewClear: function() {
			...
			return this._off(lSingleRowView$), this._off(lReportViewButton$), lSingleRowView$.html(""), lSingleRowView$
		},
		_singleRowViewControl: function() {
			...
			this.ajaxBusy || (lColumnsDisplayed$.show(), lColumnsOther$.show(), lIsExcludeNulls && lIsShowDisplayedOnly ? (lColumnsNull$.hide(), lColumnsOther$.hide()) : lIsExcludeNulls ? lColumnsNull$.hide() : lIsShowDisplayedOnly && lColumnsOther$.hide(), this.currentAction = "CHANGE_DETAIL_OPTION", this.suppressUpdate = !0, this._action("CHANGE_DETAIL_OPTION", {
		...
		_singleRowViewShow: function(pData) {
			...
			this._getElement("full_view").hide(), this._getElement("data_panel").val(""), this._getElement("dialog_js").html(""), lSingleRowView$.html(pData).show(), lSingleRowView$.find(":focusable")[0].focus(), this.options.detailLink && (this._on(this._getElement("report_view"), {
				click: function(pEvent) {
		...
		_sortWidgetHide: function() {
			var lSortWidgetRows$, lSortWidgetSearchField$, lLastColumn$, lSortWidget$ = this._getElement("sort_widget");
			lSortWidget$.is(":visible") && (lSortWidgetRows$ = this._getElement("sort_widget_rows"), lSortWidgetSearchField$ = this._getElement("sort_widget_search_field"), lLastColumn$ = $("[data-column=" + this.lastColumnId + "]", this.element), lSortWidget$.removeAttr("style").css({
				position: "absolute",
				display: "none"
			}), lLastColumn$.closest(".a-IRR-header").removeClass("is-active"), lSortWidgetRows$.html(""), $("button.a-IRR-sortWidget-button", this.element).removeClass("is-active"), this._off(lSortWidget$), this._off(lSortWidgetRows$), this._off(lSortWidgetSearchField$))
		},
		_sortWidgetSearch: function(pValue, pDoContains) {
			var lOperator = pDoContains ? "contains" : "=",
				lTemp = [this.currentColumnId, lOperator, pValue, "", ""];
			this._sortWidgetHide(), this._action("COL_FILTER", {
				f01: lTemp
			})
		},
		_sortWidgetShow: function(pData) {
			function _sortWidgetFocusableFlags(pExcludeFirst) {
				...
				pExcludeFirst || (lSortWidgetFirstFocusable$ = lFocusableElements$.first(), lSortWidgetFirstFocusable$.data("first", !0)), lSortWidgetLastFocusable$ = lFocusableElements$.last(), lSortWidgetLastFocusable$.data("last", !0)
			}
			var i, lSortWidgetFirstFocusable$, lSortWidgetLastFocusable$, lRowLength, lDisplay, lData = $.parseJSON(pData),
			...
			if (this.computationId = lData.dialog.id, lSortWidgetHelp$.hide(), $("button." + C_IRR_SORT_WIDGET_BUTTON, lSortWidget$).parent().show(), $.each(lData.dialog.hide, function(i, val) {
				$(getIdSelector(val), lSortWidget$).hide()
			}), lData.dialog.leadSortDir && ("ASC" === lData.dialog.leadSortDir ? this._getElement("sort_widget_action_up").find("button.a-IRR-sortWidget-button").addClass("is-active") : "DESC" === lData.dialog.leadSortDir && this._getElement("sort_widget_action_down").find("button.a-IRR-sortWidget-button").addClass("is-active")), lData.dialog.uv) {
				for (lRowLength = lData.dialog.row.length, i = 0; lRowLength > i; i++) lData.dialog.row[i].R && (lDisplay = lData.dialog.row[i].D ? lData.dialog.row[i].D : lData.dialog.row[i].R, out.markup("<a").attr("href", "#").attr("data-return-value", lData.dialog.row[i].R).attr("class", "a-IRR-sortWidget-row").markup(">").markup(lDisplay).markup("</a>"));
				lSortWidgetRows$.append(out.toString()), lSortWidgetSearch$.show(), lSortWidgetSearchField$.val("")
			} else lSortWidgetSearch$.hide();
			lSortWidget$.show().position({
				my: "left top",
				at: "left bottom",
				of: lCurrentColumn$
			}), lCurrentColumn$.closest(".a-IRR-header").addClass("is-active"), this.lastColumnId = this.currentColumnId, this._on(lSortWidget$, {
				...
				keydown: function(pEvent) {
					if (pEvent.which === $.ui.keyCode.ESCAPE && (this._sortWidgetHide(), pEvent.preventDefault(), lCurrentColumn$.focus()), pEvent.which === $.ui.keyCode.TAB) {
						var lGoingBackwards = !!pEvent.shiftKey;
						!lGoingBackwards && $(pEvent.target).data("last") && (pEvent.preventDefault(), lSortWidgetFirstFocusable$.focus()), lGoingBackwards && $(pEvent.target).data("first") && (pEvent.preventDefault(), lSortWidgetLastFocusable$.focus())
					}
				}
			}), this._on(lSortWidgetRows$, {
				"click a": function(pEvent) {
					this._sortWidgetSearch($(pEvent.currentTarget).data("returnValue")), pEvent.preventDefault()
				}
			}), this._on(lSortWidgetSearchField$, {
				keyup: function(pEvent) {
					lSortWidgetLastFocusable$.data("last", !1), $d_Find(this._getId("sort_widget_rows"), pEvent.target.value, "a"), _sortWidgetFocusableFlags(!0)
				},
				keydown: function(pEvent) {
					pEvent.which === $.ui.keyCode.ENTER && (this._sortWidgetSearch($(pEvent.target).val(), !0), pEvent.preventDefault())
				}
			}), this._dialogCheck(null, this._sortWidgetHide), _sortWidgetFocusableFlags(), lSortWidgetFirstFocusable$.focus()
		},
		...
		_utilGetFormElAttributes: function(pContainerId) {
			var lContainerId, lIds = [],
				lValues = [];
			return lContainerId = pContainerId ? pContainerId : this._getId("dialog_js"), this._utilGetFormEls(lContainerId).each(function(i, pElement) {
				lIds.push(pElement.id), lValues.push($v(pElement.id))
			}), {
				ids: lIds,
				values: lValues
			}
		...
		_utilGetFormElValues: function(pIdArray) {
			var i, lValues = [];
			for (i = 0; i < pIdArray.length; i++) lValues.push($v(pIdArray[i]));
			return lValues
		},
